...
export interface MergedAccount {
  ...
  sources: Array<{ name: string; inUse: boolean }>;
  inUse: boolean;
  email?: string;
  handle?: string;
  alias?: string;
...
export function maskEmail(email: string): string {
  ...
  if (at < 1) return maskHandle(email);
...
export function accountLabel(account: Pick<MergedAccount, 'alias' | 'email' | 'handle'>): string | null {
 ...
 */
export function mergeLogins(providerId: string, logins: Login[]): MergedAccount[] {
  interface Group { keys: Set<string>; tokens: Set<string>; logins: Login[] }
  const groups: Group[] = [];
  for (const login of logins) {
    ...
    const target: Group = matches[0] ?? { keys: new Set(), tokens: new Set(), logins: [] };
    ...
    for (const other of matches.slice(1)) {
      for (const value of other.keys) target.keys.add(value);
      for (const value of other.tokens) target.tokens.add(value);
  ...
  const lasting = (login: Login) => login.credentials.expiresAt ?? Number.POSITIVE_INFINITY;
  return groups.map(group => {
    ...
    for (const login of group.logins) sources.set(login.source, (sources.get(login.source) ?? false) || login.inUse === true);
    const first = (field: 'email' | 'handle' | 'alias') => group.logins.find(login => login[field])?.[field];
    return {
...
export function accountMatches(account: MergedAccount, selector: string): boolean {
