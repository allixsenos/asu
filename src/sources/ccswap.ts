 ...
 */
export function ccswapRoot(context: LocalContext): string {
...
function slots(raw: unknown): Array<[string, Entry]> {
  return Object.entries(optionalObject(credentialObject(raw).accounts))
    .filter((pair): pair is [string, Entry] => /^\d+$/.test(pair[0]) && !!pair[1] && typeof pair[1] === 'object' && !Array.isArray(pair[1]));
}

/** One Claude slot: its backup, and the profile `ccswap run` made for it, which usually holds a fresher token. */
async function claudeSlot(context: LocalContext, root: string, number: string, record: Entry): Promise<Login[]> {
  ...
  const logins: Login[] = [];
  const add = (credentials: Credentials | null) => { if (credentials) logins.push({ credentials, source: `ccswap slot ${number}`, ...identity }); };
  ...
  if (encoded !== null) add(parseCredentials(JSON.parse(Buffer.from(encoded.trim(), 'base64').toString('utf8'))));
  else if (context.platform === 'darwin') {
    const raw = await context.keychain('claude-swap', undefined, `account-${number}-${email}`);
    if (raw) add(parseCredentials(JSON.parse(raw)));
  }
  const profile = await context.readJson(join(root, 'sessions', `${number}-${slugifyEmail(email)}`, '.credentials.json'));
  if (profile != null) add(parseCredentials(profile));
  return logins;
}

/** One Codex slot: a verbatim copy of Codex CLI's auth.json. */
async function codexSlot(context: LocalContext, root: string, number: string, record: Entry): Promise<Login[]> {
 ...
 */
export const ccswapSource: LoginSource = {
  id: 'ccswap',
  async list(context) {
    const root = ccswapRoot(context);
    const found: Array<{ providerId: string; login: Login }> = [];
    const read = async (providerId: string, sequence: string, slot: (number: string, record: Entry) => Promise<Login[]>) => {
      ...
      for (const [number, record] of slots(raw)) {
        ...
        try { for (const login of await slot(number, record)) found.push({ providerId, login }); } catch { /* Skip this slot. */ }
      }
    };
