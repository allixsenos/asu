...
export class UsageCache {
  private readonly memory = new Map<string, ProviderUsage>();
  private readonly pending = new Map<string, Promise<ProviderUsage>>();
  readonly warnings = new Set<string>();
  constructor(private readonly directory?: string, private readonly now = Date.now) {}

  async getOrFetch(key: string, load: () => Promise<ProviderUsage>, fresh = false): Promise<ProviderUsage> {
    if (!/^[a-f0-9]{64}$/.test(key)) throw new Error('Invalid cache key');
    const pending = this.pending.get(key);
    if (pending) return structuredClone(await pending);
    const startedAt = this.now();
    const current = this.memory.get(key);
    if (!fresh && current && this.valid(current)) return { ...structuredClone(current), cached: true };
  ...
  private valid(data: ProviderUsage): boolean {
  ...
  private async read(path: string, fresh: boolean, startedAt: number): Promise<ProviderUsage | null> {
    try {
      ...
      if (!result.success || !this.valid(result.data)) return null;
      if (fresh && Date.parse(result.data.fetchedAt) < startedAt) return null;
      return { ...result.data, cached: true };
    } catch { return null; }
  }
  private async load(key: string, load: () => Promise<ProviderUsage>, fresh: boolean, startedAt: number): Promise<ProviderUsage> {
    if (!this.directory) return load();
    ...
    try {
      ...
      if (!stat.isDirectory() || (process.getuid && stat.uid !== process.getuid())) throw new Error();
    ...
    try {
      ...
      if (!compromised) {
        ...
        } catch { this.warnings.add('Could not save the usage cache.'); }
