...
export async function opencodeEntry(context: LocalContext, ids: string[]): Promise<{ id: string; entry: OpencodeEntry } | null> {
  ...
  for (const id of ids) {
    ...
    if (item.type === 'api') {
      ...
      if (key) return { id, entry: { type: 'api', key } };
    }
  }
  return null;
}
...
export function oauthCredentials(entry: Extract<OpencodeEntry, { type: 'oauth' }>, token = entry.access): Credentials {
