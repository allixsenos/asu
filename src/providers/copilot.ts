...
export function normalizeCopilot(payload: unknown): UsageData {
  ...
  for (const [name, value] of Object.entries(optionalObject(raw.quota_snapshots))) {
    ...
    if (remainingPercent !== undefined && remainingPercent > 100) throw new UsageError('invalid_response');
    ...
    if (percentUsed === null && used === undefined && limit === undefined && !unlimited) continue;
