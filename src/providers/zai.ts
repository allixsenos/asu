...
function envelope(payload: unknown): unknown {
  ...
  if (raw.success === false || (raw.code !== undefined && ![0, 200, '0', '200'].includes(raw.code as number | string)))
    throw new UsageError(raw.code === 401 || raw.code === 403 ? 'unauthorized' : 'provider_error');
  return raw.data;
}
export function normalizeZaiPlan(payload: unknown): UsageData {
  const data = emptyUsage();
  const subscriptions = list(envelope(payload)).map(object);
  const names = subscriptions.map(sub => string(sub.productName)).filter((name): name is string => Boolean(name));
  data.planLabel = [...new Set(names)].join(' / ') || null;
  for (const sub of subscriptions) {
    for (const field of ['status', 'valid', 'purchaseTime']) {
      const value = string(sub[field]);
      if (value) data.details.push({ label: `${string(sub.productName) ?? 'Subscription'} ${field}`, value });
    }
  }
  return requireUsage(data);
}
export function normalizeZaiQuota(payload: unknown): UsageData {
  ...
  for (const [index, value] of list(raw.limits).entries()) {
    ...
    if (quota.percentage == null) continue;
...
export const zai: Provider = {
  ...
  async fetchUsage(context, credentials) {
    ...
    if (plan.status === 'rejected' && quota.status === 'rejected') throw quota.reason;
    const data = plan.status === 'fulfilled' ? plan.value : emptyUsage();
    if (quota.status === 'fulfilled') data.windows = quota.value.windows;
    else data.details.push({ label: 'Quota', value: 'Usage figures could not be retrieved' });
    if (plan.status === 'rejected') data.details.push({ label: 'Plan', value: 'Plan metadata could not be retrieved' });
    return data;
  },
};
