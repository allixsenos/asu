...
export function normalizeKimi(payload: unknown): UsageData {
  ...
  function add(id: string, label: string, value: unknown) {
    const row = { ...object(value) }, remaining = nonnegative(row.remaining), limit = nonnegative(row.limit);
    if (row.used == null && remaining !== undefined && limit !== undefined) row.used = Math.max(0, limit - remaining);
    data.windows.push(countWindow(id, label, row, row.resetTime ?? row.resetAt ?? row.reset_time ?? row.reset_at));
  }
  if (raw.usage != null) add('weekly', 'Weekly', raw.usage);
