...
- `dist/` is committed so that a Git install needs no build step. Run `npm run build` and commit `dist/` together with the source change. CI fails when they differ.
