...
export function normalizeGrok(payload) {
    ...
    if (percentUsed !== null)
        data.windows.push({ id: 'included', label: string(current.type)?.includes('WEEKLY') ? 'Weekly' : 'Billing period', percentUsed, resetsAt });
    if (used !== undefined || limit !== undefined)
        data.balances.push({ id: 'included', label: 'Included allowance', unit: 'USD',
    ...
    if (prepaid !== undefined)
        data.balances.push({ id: 'prepaid', label: 'Prepaid balance', unit: 'USD', remaining: prepaid / 100 });
    if (config.isUnifiedBillingUser === true)
        data.details.push({ label: 'Billing', value: 'Shared subscription usage' });
...
export const grok = {
    ...
    async listLogins(context) {
        ...
        const cli = (token) => logins.push({ credentials: { token }, source: 'Grok CLI', inUse: !logins.some(login => login.source === 'Grok CLI') });
        if (legacy)
            cli(legacy);
