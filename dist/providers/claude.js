...
function scopeId(scope) {
    return ['model', 'surface'].flatMap(dimension => {
...
export function normalizeClaude(payload, metadata = {}) {
    ...
    for (const [key, label] of [['five_hour', '5 hours'], ['seven_day', 'Weekly']]) {
        if (raw[key] == null)
            continue;
    ...
    for (const [key, value] of Object.entries(raw)) {
        if (!key.startsWith('seven_day_') || value == null)
            continue;
        try {
        ...
        for (const value of section) {
            try {
                ...
                if (entry.kind !== 'weekly_scoped')
                    continue;
                const rawScope = object(entry.scope), scope = {};
                for (const dimension of ['model', 'surface']) {
                    const part = optionalObject(rawScope[dimension]);
                    const id = string(part.id), name = string(part.display_name) ?? id;
                    if (name)
                        scope[dimension] = { id, name };
                }
                if (!scope.model && !scope.surface) {
                    skipped++;
                    continue;
                }
                ...
                if (index < 0)
                    ...
                    scoped[index] = { ...next, percent: next.percent ?? scoped[index].percent, reset: next.reset ?? scoped[index].reset };
            }
            catch {
                skipped++;
            }
    ...
    if (typeof extra.is_enabled === 'boolean')
        data.details.push({ label: 'Extra usage', value: extra.is_enabled ? 'Enabled' : 'Disabled' });
    requireUsage(data); // A locally known plan must not make an empty API response appear valid.
    if (skipped)
        data.details.push({ label: 'Response warning', value: `${skipped} unrecognized scoped limits omitted` });
