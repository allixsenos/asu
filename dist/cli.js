...
export async function run(args = process.argv.slice(2)) {
    let format = process.stdout.isTTY ? 'bars' : 'plain';
    try {
        ...
        if (formats.length > 1 || formats.some(value => !['plain', 'table', 'bars', 'json'].includes(value)))
            throw new Error('Choose one output format: plain, table, bars, or json.');
        ...
        if (values.account !== undefined && !report.providers.length)
            throw new Error('No account matches --account. Run asu without it to see the account labels.');
